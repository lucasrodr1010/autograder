...
class AutograderGUI:
    def __init__(self, root):
        ...
        self.strict_stdout = tk.BooleanVar(value=True)
    ...
    def setup_gui(self):
        ...
        # Update inner frame width when canvas changes size (window resized)
        def _on_canvas_config(event):
            self.test_canvas.itemconfigure(inner_window_id, width=event.width)
            
        self.test_canvas.bind("<Configure>", _on_canvas_config)
        ...
        ttk.Checkbutton(options_frame, text="Grade Standard Output (uncheck to ONLY grade file output)", variable=self.strict_stdout).pack(side=tk.LEFT, padx=(0, 10))
    ...
    def run_autograder_thread(self):
        ...
        try:
            ...
            for i, student_path in enumerate(student_files):
                ...
                self.compare_results(base_results, student_results, student_name, test_cases)
    ...
    def run_solution(self, path, test_cases, name):
        ...
        for i, test_case in enumerate(test_cases):
            try:
                ...
                output, file_contents = self.execute_script(main_script, test_case["input"], path, expected_filename)
                results.append({
                    "test_case": i + 1,
    ...
    def find_main_script(self, folder_path):
        ...
        # Prioritize files that contain the folder name (e.g., folder 'LR' -> 'LR_ICA5.py')
        folder_name = folder.name.lower()
        
        # Filter out skipped files
        valid_files = [f for f in python_files if f.name not in skip_names and not any(f.name.startswith(s) for s in skip_names if not s.endswith(".py"))]
        
        # Sort for determinism
        valid_files.sort(key=lambda x: x.name)
        
        # First pass: look for exact prefix match with folder name (e.g. 'jc_ica5.py' in 'JC' folder)
        for file in valid_files:
            if file.name.lower().startswith(folder_name):
                return str(file)
                
        # Second pass: look for any file containing 'ica' or 'pca' ignoring case, as a fallback main module indicator
        for file in valid_files:
            name_lower = file.name.lower()
            if 'ica' in name_lower or 'pca' in name_lower:
                return str(file)
                
        # If no clear main script found, return the first valid file
        return str(valid_files[0]) if valid_files else None
        
    def execute_script(self, script_path, input_data, working_dir, expected_filename=""):
        ...
        Args:
            ...
            expected_filename: Filename to read as test case output
            
        Returns:
            tuple: (Stdout content, file_contents dict)
        ...
        try:
            ...
            # Execute in a temporary directory to completely isolate file generation
            with tempfile.TemporaryDirectory() as temp_dir:
                # Need to copy the target script and any other files from its folder to the temp dir
                source_dir = script_dir if self.mode.get() == "file" else working_dir
                ...
                # Copy global assets from assignment folder (if any) to temp dir, acting as global overrides
                # This ensures clean database files (.csv, .txt) are used instead of dirty student copies
                assignment_root = self.assignment_folder_path.get()
                if assignment_root and os.path.exists(assignment_root):
                    if os.path.isdir(assignment_root):
                        for item in os.listdir(assignment_root):
                            s = os.path.join(assignment_root, item)
                            d = os.path.join(temp_dir, item)
                            if os.path.isfile(s) and not s.endswith(".py"):
                                shutil.copy2(s, d)
                ...
                if result.returncode != 0:
                    if "EOFError" in result.stderr:
                        return result.stdout, {"error": "Script requested more input() than provided (EOFError).", "raw_stderr": result.stderr}
                    elif "NameError" in result.stderr or "TypeError" in result.stderr or "AttributeError" in result.stderr:
                        return result.stdout, {"error": f"Student Code Error: {result.stderr.strip().split()[-1]}", "raw_stderr": result.stderr}
                    raise Exception(f"Runtime error: {result.stderr}")
                    
                # Read expected files from the temp directory before it gets cleaned up
                file_contents = {}
                if expected_filename and expected_filename.strip():
            ...
            return result.stdout, file_contents
    ...
    def compare_results(self, base_results, student_results, student_name, test_cases=None):
        ...
        Args:
            ...
            test_cases: List of test cases for expected file overrides
        """
        ...
        for i, (base_result, student_result) in enumerate(zip(base_results, student_results)):
            ...
            # Compare stdout
            outputs_match = True
            if self.strict_stdout.get():
                if not self.compare_outputs(base_result["output"], student_result["output"]):
                    outputs_match = False
            ...
            expected_override = test_case.get("expected_file_content", "")
            expected_filename = test_case.get("expected_filename", "").strip()
            
            # Fetch files dicts safely for scope access
            # Fallback to empty dicts if None
            base_files = base_result.get("files") or {}
            student_files = student_result.get("files") or {}
            
            if expected_override and expected_filename:
                student_content = student_files.get(expected_filename)
                
                if student_content is None:
                    files_match = False
                    file_mismatch_details.append(f"Missing expected output file: {expected_filename}")
                elif isinstance(student_content, str):
                    if not self.compare_outputs(expected_override, student_content):
                        files_match = False
                        file_mismatch_details.append(f"File contents for '{expected_filename}' do not match expected override text")
                        
                        # Populate base_files and base_result explicitly for diff viewing
                        base_files[expected_filename] = expected_override
                        if "files" not in base_result or not base_result["files"]:
                            base_result["files"] = {}
                        base_result["files"][expected_filename] = expected_override

            elif base_files:
                for filename, base_content in base_files.items():
                    student_content = student_files.get(filename)
                    if student_content is None and base_content is not None:
                        files_match = False
                        file_mismatch_details.append(f"Missing expected output file: {filename}")
                    elif isinstance(base_content, str) and isinstance(student_content, str):
                        if not self.compare_outputs(base_content, student_content):
                            files_match = False
                            file_mismatch_details.append(f"File contents for '{filename}' do not match")
                    elif base_content != student_content:
                        files_match = False
                        file_mismatch_details.append(f"File contents for '{filename}' do not match")
            
            if outputs_match and files_match:
                passed_tests += 1
            else:
                ...
                if not files_match:
                    for detail in file_mismatch_details:
                        errors.append(f"Test {i+1}: File mismatch - {detail}")
                    self.update_results(f"\n--- FILE OUTPUT DIFFERENCES: {student_name} - Test {i+1} ---\n")
                    for detail in file_mismatch_details:
                        self.update_results(f"  {detail}\n")
                        
                        # Show diff if both files exist and are text
                        if "do not match" in detail:
                            filename = detail.split("'")[1]
                            b_content = base_files[filename]
                            s_content = student_files[filename]
                            
                            if isinstance(b_content, str) and isinstance(s_content, str):
                                self.update_results("  Diff:\n")
                                diff = difflib.unified_diff(
                                    self.normalize_output(b_content), 
                                    self.normalize_output(s_content),
                                    fromfile='Expected File', tofile='Got File',
                                    lineterm=''
                                )
                                for line in diff:
                                ...
                                self.update_results("  (Binary files differ)\n")
                                
                    self.update_results("--- END FILE DIFFERENCES ---\n")
...
class TestCaseFrame(ttk.Frame):
    ...
    def __init__(self, parent, index, autograder_gui):
        ...
        # Row 0: Header and Remove button
        header = ttk.Frame(self)
        header.grid(row=0, column=0, columnspan=2, sticky=(tk.W, tk.E))
        ttk.Label(header, text=f"Test Case {index + 1}:").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(header, text="Remove", command=self.remove).pack(side=tk.RIGHT, padx=(5, 0))
        
        # Row 1: Standard Input
        ttk.Label(self, text="Standard Input:").grid(row=1, column=0, sticky=tk.NW, padx=(0, 5), pady=2)
        self.input_text = tk.Text(self, height=3, width=25)
        self.input_text.grid(row=1, column=1, sticky=(tk.W, tk.E), padx=(5, 5), pady=2)
        self.input_text.configure(
            bg=UIConfig.ENTRY_BG, fg=UIConfig.BORDER_COLOR, insertbackground=UIConfig.FG_COLOR,
            selectbackground=UIConfig.SELECTION_BG, font=UIConfig.MONO_FONT,
            highlightthickness=1, highlightbackground=UIConfig.BORDER_COLOR,
            highlightcolor=UIConfig.ACCENT_COLOR, relief="flat"
        )
        ...
        self.placeholder_input = "Enter test input here..."
        self.has_content_input = False

        # Row 1 (Cols 2 & 3): Expected File Output
        ttk.Label(self, text="File Check\n(Optional):").grid(row=1, column=2, sticky=tk.NW, padx=(10, 5), pady=2)
        
        file_frame = ttk.Frame(self)
        file_frame.grid(row=1, column=3, sticky=(tk.W, tk.E), padx=(5, 5), pady=2)
        
        # Inner top: filename
        filename_frame = ttk.Frame(file_frame)
        filename_frame.pack(side=tk.TOP, fill=tk.X, pady=(0, 2))
        ttk.Label(filename_frame, text="Filename:").pack(side=tk.LEFT)
        self.expected_filename = ttk.Entry(filename_frame, width=15)
        self.expected_filename.pack(side=tk.LEFT, padx=(5, 0), fill=tk.X, expand=True)
        
        # Inner bottom: text
        self.expected_file_text = tk.Text(file_frame, height=3, width=20)
        self.expected_file_text.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        self.expected_file_text.configure(
            bg=UIConfig.ENTRY_BG, fg=UIConfig.BORDER_COLOR, insertbackground=UIConfig.FG_COLOR,
            selectbackground=UIConfig.SELECTION_BG, font=UIConfig.MONO_FONT,
            highlightthickness=1, highlightbackground=UIConfig.BORDER_COLOR,
            highlightcolor=UIConfig.ACCENT_COLOR, relief="flat"
        )
        self.expected_file_text.insert(tk.END, "Enter expected file content here...")
        self.expected_file_text.bind("<FocusIn>", self.on_focus_in)
        self.expected_file_text.bind("<FocusOut>", self.on_focus_out)
        self.placeholder_file = "Enter expected file content here..."
        self.has_content_file = False
    ...
    def on_focus_in(self, event):
        ...
        widget = event.widget
        if widget == self.input_text and not self.has_content_input:
            self.input_text.delete(1.0, tk.END)
            self.input_text.configure(fg=UIConfig.ENTRY_FG)
        elif widget == self.expected_file_text and not self.has_content_file:
            self.expected_file_text.delete(1.0, tk.END)
            self.expected_file_text.configure(fg=UIConfig.ENTRY_FG)
    ...
    def on_focus_out(self, event):
        ...
        if widget == self.input_text:
            if not self.input_text.get(1.0, tk.END).strip():
                self.input_text.insert(tk.END, self.placeholder_input)
                self.input_text.configure(fg=UIConfig.BORDER_COLOR)
                self.has_content_input = False
            else:
                self.has_content_input = True
        elif widget == self.expected_file_text:
            if not self.expected_file_text.get(1.0, tk.END).strip():
                self.expected_file_text.insert(tk.END, self.placeholder_file)
                self.expected_file_text.configure(fg=UIConfig.BORDER_COLOR)
                self.has_content_file = False
            else:
                self.has_content_file = True

    def is_valid(self):
        """Check if test case has valid content."""
        content = self.input_text.get(1.0, tk.END).strip()
        return bool(content) and content != self.placeholder_input
        
    def get_data(self):
        """Get the test case data."""
        input_text = self.input_text.get(1.0, tk.END).strip()
        if input_text == self.placeholder_input:
            input_text = ""
            
        file_text = self.expected_file_text.get(1.0, tk.END).strip()
        if file_text == self.placeholder_file:
            file_text = ""
            
        filename = self.expected_filename.get().strip()
            
        return {
            "input": input_text.split('\n') if input_text else [],
            "expected_file_content": file_text,
